  ...
  implements OnInit, OnDestroy
{
  ...
  exampleDatabase?: CompanyManagementService;
  dataSource!: ExampleDataSource;
  selection = new SelectionModel<AdvanceTable>(true, []);
  id?: number;
  advanceTable?: CompanyManagementService;
  deldata: any[] = [];
  addBoolean: boolean = false;
  private shouldDisplayCmanagementSubscription!: Subscription;
  storeAllFile: any[] = [];
  ...
  toggleColumn(column: string, checked: boolean) {
    ...
    if (column === 'cname') {
      this.displayCname = checked;
    } else if (column === 'ccode') {
      this.displayCcode = checked;
    }
  }
  ...
  ngOnInit() {
    this.shouldDisplayCmanagementSubscription =
      this.headerService.shouldDisplayCmanagement$.subscribe((shouldDisplay) => {
        if (shouldDisplay) {
          this.addNew();
        }
      });
  ...
  override ngOnDestroy() {
    ...
    this.shouldDisplayCmanagementSubscription.unsubscribe();
  }
  ...
  addNew() {
    ...
    const dialogRef = this.dialog.open(CompanyManagementFormComponent, {
      data: {
  ...
  public loadData() {
    this.exampleDatabase = new CompanyManagementService(this.httpClient);
  ...
  exportExcel() {
    const exportData = this.dataSource.filteredData.map((x) => {
      ...
      if (!this.displayCname == false) row[this.translateService.instant("Company Name")] = x.cname;
      if (this.displayCcode) row[this.translateService.instant("Company Code")] = x.ccode;
      return row;
    });
  
    TableExportUtil.exportToExcel(exportData, "Company Management");
  }
  ...
  exportCSV() {
    ...
    TableExportUtil.exportToCSV(exportData, "Company Management");
  }
  // context menu
...
export class ExampleDataSource extends DataSource<AdvanceTable> {
  ...
  constructor(
    public exampleDatabase: CompanyManagementService,
    ...
    return data.sort((a, b) => {
      ...
      switch (this._sort.active) {
        ...
        case "cname":
          ...
          break;
        case "ccode":
          [propertyA, propertyB] = [a.ccode, b.ccode];
          break;
      }
