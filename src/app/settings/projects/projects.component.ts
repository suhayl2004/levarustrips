  ...
  implements OnInit, OnDestroy
{
  ...
  exampleDatabase?: ProjectsService;
  dataSource!: ExampleDataSource;
  selection = new SelectionModel<AdvanceTable>(true, []);
  id?: number;
  advanceTable?: ProjectsService;
  deldata: any[] = [];
  addBoolean: boolean = false;
  private shouldDisplayProjectsSubscription!: Subscription;
  storeAllFile: any[] = [];
  ...
  toggleColumn(column: string, checked: boolean) {
    ...
    if (column === 'company') {
      this.displayCompany = checked;
    } else if (column === 'pcode') {
      this.displayPcode = checked;
    } else if (column === 'pname') {
      this.displayPname = checked;
    }
  }
  ...
  ngOnInit() {
    this.shouldDisplayProjectsSubscription =
      this.headerService.shouldDisplayProjects$.subscribe((shouldDisplay) => {
        if (shouldDisplay) {
          this.addNew();
        }
      });
  ...
  override ngOnDestroy() {
    ...
    this.shouldDisplayProjectsSubscription.unsubscribe();
  }
  ...
  public loadData() {
    this.exampleDatabase = new ProjectsService(this.httpClient);
  ...
  exportExcel() {
    const exportData = this.dataSource.filteredData.map((x) => {
      ...
      if (!this.displayCompany == false) row[this.translateService.instant("Company")] = x.company;
      if (this.displayPcode) row[this.translateService.instant("Project Code")] = x.pcode;
      if (this.displayPname) row[this.translateService.instant("Project Name")] = x.pname;
      return row;
    });
  
    TableExportUtil.exportToExcel(exportData, "Projects");
  }
  ...
  exportCSV() {
    ...
    TableExportUtil.exportToCSV(exportData, "Projects");
  }
  // context menu
...
export class ExampleDataSource extends DataSource<AdvanceTable> {
  ...
  constructor(
    public exampleDatabase: ProjectsService,
    ...
    return data.sort((a, b) => {
      ...
      switch (this._sort.active) {
        ...
        case "company":
          ...
          break;
        case "pcode":
          [propertyA, propertyB] = [a.pcode, b.pcode];
          break;
        case "pname":
