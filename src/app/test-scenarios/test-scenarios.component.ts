...
import { AdvanceTable } from "./test-scenarios.model";
...
@Component({
  ...
  styleUrl: "./test-scenarios.component.scss",
  providers: [{ provide: MAT_DATE_LOCALE, useValue: "en-GB" }],
})
export class TestScenariosComponent
  extends UnsubscribeOnDestroyAdapter
  implements OnInit, OnDestroy
{
  @Input() hideAddButton: boolean = false;
  displayedColumns = ["select", "tscenario", "tcases", "actions"];
  exampleDatabase?: TestScenariosService;
  @Input() dataSource!: ExampleDataSource1;
  selection = new SelectionModel<AdvanceTable>(true, []);
  id?: number;
  advanceTable?: TestScenariosService;
  deldata: any[] = [];
  private shouldDisplayTScenariosSubscription!: Subscription;
  typeSelected: string;
  private startX!: number;
  private startWidth!: number;
  private currentResizer!: HTMLElement;
  displayTscenario = true;
  displayTcases = true;
  toggleColumn(column: string, checked: boolean) {
    console.log(`Toggling ${column}: ${checked}`);
    if (column === "tscenario") {
      this.displayTscenario = checked;
    } else if (column === "tcases") {
      this.displayTcases = checked;
    }
  }
  ...
  openNewTab(row: any): void {
    ...
    this.advanceTableService.selectedItems$.subscribe((items) => {
      ...
      this.advanceTableService.updateSelectedItems(this.selectedItems);
      this.router.navigate(["/test-scenarios-detail-screen"], {
        state: { rowData: row, selectedItems: this.selectedItems },
      });
    });
  }
  ...
  ngOnInit() {
    this.advanceTableService.data$.subscribe((data) => {
      console.log("Data in TestScenariosComponent:", data);
    ...
    this.shouldDisplayTScenariosSubscription =
      this.headerService.shouldDisplayTestScenarios$.subscribe(
        (shouldDisplay) => {
          if (shouldDisplay) {
            this.addNew();
          }
  ...
  override ngOnDestroy() {
    ...
    this.shouldDisplayTScenariosSubscription.unsubscribe();
  }
  refresh() {
    this.loadData();
  }
  ...
  addNew() {
    ...
    const dialogRef = this.dialog.open(TestScenariosFormComponent, {
      data: {
  ...
  exportExcel() {
    const exportData = this.dataSource.filteredData.map((x) => {
      ...
      if (!this.displayTscenario == false)
        row[this.translateService.instant("Test Scenario")] = x.tscenario;
      if (!this.displayTcases == false)
        row[this.translateService.instant("Test Cases")] = x.tcases;
      return row;
    });

    TableExportUtil.exportToExcel(exportData, "Test Scenarios");
  }
  ...
  exportCSV() {
    ...
    TableExportUtil.exportToCSV(exportData, "Test Scenarios");
  }
  // context menu
...
export class ExampleDataSource1 extends DataSource<AdvanceTable> {
    ...
    return data.sort((a, b) => {
      ...
      switch (this._sort.active) {
        case "id":
          ...
          break;
        case "tscenario":
          [propertyA, propertyB] = [a.tscenario, b.tscenario];
          break;
        case "tcases":
          [propertyA, propertyB] = [a.tcases, b.tcases];
          break;
      }
